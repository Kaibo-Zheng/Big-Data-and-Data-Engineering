...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
Usage: bash scripts/run_deberta_lora.sh [options]
...
Options:
  ...
  --grad-accum N          Gradient accumulation, default 16
  --batch-size N          Train batch size, default 1
...
if [[ -z "$LOG_PATH" ]]; then
  LOG_PATH="logs/deberta_lora_$(date +%Y%m%d_%H%M%S).log"
fi
